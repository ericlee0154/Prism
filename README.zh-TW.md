...
交易日的歷史類比 forecast，回報 10%、50%、90% 報酬與價格點位、
正報酬比例及樣本數。股票、日期或滑桿變更後會自動重算；整段區間也能依
指定天數向左或向右平移，方便逐日檢查 metrics 的連續性。

若歷史分析 cutoff 後已有儲存日線，只有 Forecast 真值驗證區會讀取它們，
並列出目標日及其前後五個交易日。區間外資料不會進入 Metrics、上方價格
序列或歷史類比生成。Walk-forward 回測完成後另提供依日期與股票排列的
20 日年化已實現波動 3D 曲面。少於十個合格樣本時不產生 forecast 數值。
